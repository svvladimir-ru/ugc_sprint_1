...
from auth_utils.utils.http_client import HTTPClient, http_client
from auth_utils.core.config import AUTH_TOKEN_VALIDATION_URL, AUTH_PERMISSIONS_AND_TOKEN_VALIDATION_URL, AUTH_SECRET_KEY
...
class AuthRequired:
    """
        ...
        :param validate_locally
            If True, token is validated locally. Both token signature and token expiration are checked.
        """
    _http_bearer = HTTPBearer(auto_error=False)

    def __init__(self, *permissions: str, condition: Optional[dict] = None,
                 token_optional=True, permissions_optional=True, validate_locally=False):
    ...
    async def __call__(self, request: Request,
        ...
        if self.validate_locally and not self.permissions and not self.condition:
            return self._validate_locally(token)
        else:
            return await self._validate(token, client)

    def _validate_locally(self, token: str) -> AuthInfo:
        try:
            decoded = jwt.decode(token, key=AUTH_SECRET_KEY, algorithms=['HS256', 'RS256'],
                                 leeway=timedelta(seconds=30),
                                 options={'verify_signature': True,
                                          'verify_exp': True})
        except jwt.exceptions.InvalidSignatureError:
            raise HTTPException(status_code=HTTP_401_UNAUTHORIZED, detail="Invalid token signature")
        except jwt.exceptions.DecodeError:
            raise HTTPException(status_code=HTTP_401_UNAUTHORIZED, detail="Token cannot be decoded")
        ...
        user_id = decoded['sub']
        return AuthInfo(user_id=user_id)

    async def _validate(self, token: str, client: HTTPClient) -> AuthInfo:
